 ...
 */
jboolean
GetJavaHome(char *buf, jint bufsize)
{
	//currently linux only
	const char *javahome = getenv("JAVA_HOME");
	//const char *javahome = "/usr/local/java/jdk1.8.0_25";
	//const char *javahome = "/usr/lib/jvm/java-7-openjdk-amd64";
	if (javahome != NULL) {
		JLI_Snprintf(buf, bufsize, "%s", javahome);
		buf[bufsize-1] = '\0';
	} else {
		return JNI_FALSE;
	}
...
jboolean
GetJythonHome(char *buf, jint bufsize)
{
	//currently linux only
	const char *jythonhome = getenv("JYTHON_HOME");
	if (jythonhome != NULL) {
		JLI_Snprintf(buf, bufsize, "%s", jythonhome);
		buf[bufsize-1] = '\0';
	} else {
		return JNI_FALSE;
	}
