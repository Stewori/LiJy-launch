...
CreateExecutionEnvironment(int *pargc, char ***pargv,
   ...
   */
    ...
    SetExecname(jysetup->progName);//*pargv);
...
//        puts("GetJREPath result:");
//        puts(jrepath);
        /* Find Jython... */
        if (!GetJyPath(jypath, so_jypath, arch, JNI_FALSE) ) {
		  JLI_ReportErrorMessage("Error: Could not find Jython environment.");
		  exit(2);
		}
//		puts("GetJyPath result:");
//		puts(jypath);
...
/*
 * Find path to Jython based on .exe's location or registry settings.
 */
static jboolean
GetJyPath(char *path, jint pathsize, const char * arch, jboolean speculative)
{
	char* sloff;
	if (!GetJythonHome(path, pathsize)) {
		const char *execname = GetExecName();
		JLI_Snprintf(path, pathsize, "%s", execname);
		sloff = JLI_StrRChr(path, '/');
		if (!sloff) return JNI_FALSE;
		strcpy(sloff, "/jython-dev.jar\0");
		if (exists(path)) return JNI_TRUE;
		strcpy(sloff, "/jython.jar\0");
		if (exists(path)) return JNI_TRUE;
		strcpy(sloff, "/jython-standalone.jar\0");
		if (exists(path)) return JNI_TRUE;
		*sloff = '\0';
		sloff = JLI_StrRChr(path, '/');
		if (!sloff) return JNI_FALSE;
 ...
 */
const char*
SetExecname(char* progName)//char **argv)
{
    char* exec_path = NULL;
    ...
    if (exec_path == NULL) {
        exec_path = FindExecName(progName);//argv[0]);
    }
    execname = exec_path;
    return exec_path;
}
...
JVMInit(InvocationFunctions* ifn, jlong threadStackSize,
        int argc, char **argv,
        int mode, char *what, int ret, jboolean printHelp)
{
    return ContinueInNewThread(ifn, threadStackSize, argc, argv, mode, what, ret, printHelp);
}
